...
class FilterListProducts extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
        future: searchProvider.filterProducts(),
        builder: (BuildContext context,AsyncSnapshot<List<ProductModel>> list) {
          ...
          List<ProductModel> items = list.data;
          if(items.length == 0 ){
            return Container(
              padding: EdgeInsets.symmetric(horizontal: 25, vertical: 10),
              alignment: Alignment.topCenter,
              child: Text('Ups :(  no pudimos encontrar el producto que necesitas, intenta realizar otra búsqueda', style: TextStyle(fontSize: 20),),
            );
          }
